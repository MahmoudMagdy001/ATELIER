...
export interface VipBespokeCtaProps {
  settings?: SiteSettings | null
}

export default function VipBespokeCta({ settings: _settings }: VipBespokeCtaProps = {}) {
  const { t } = useTranslation('contact')
  const whatsappNum = CONTACT_INFO.whatsappRaw
  const phoneNum = CONTACT_INFO.phone
  const whatsappMsg = t('vipCta.whatsappMessage')
  ...
  return (
    ...
    >
      <div className="rounded-3xl bg-gradient-to-r from-[#7A5D2B] via-[#C4A070] to-[#E5C9A3] p-8 sm:p-10 md:p-14 text-[#1C1816] shadow-2xl flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden">
        ...
        <div className="space-y-3 text-center md:text-start max-w-xl relative z-10">
          ...
          <p className="text-xs md:text-sm font-medium text-[#26211F] leading-relaxed">
            {t('vipCta.description')}
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 shrink-0 w-full sm:w-auto relative z-10">
          <motion.a
            ...
            className="px-8 py-4 rounded-full bg-[#1C1816] text-[#F2EFE8] font-bold text-xs flex items-center justify-center gap-2.5 shadow-xl hover:bg-black transition-colors cursor-pointer"
          >
            <FaWhatsapp className="w-4 h-4 text-[#C4A070]" />
            <span>{t('vipCta.whatsappBtn')}</span>
          </motion.a>
          <motion.a
            whileHover={hoverScale}
            whileTap={tapScale}
            transition={springHover}
            href={`tel:${phoneNum}`}
            className="px-6 py-4 rounded-full bg-white/30 backdrop-blur-md text-[#1C1816] font-bold text-xs flex items-center justify-center gap-2 border border-black/10 hover:bg-white/40 transition-colors cursor-pointer"
          >
            <FaPhone className="w-3.5 h-3.5" />
            <span>{t('vipCta.phoneBtn')}</span>
          </motion.a>
        </div>
      </div>
    </motion.section>

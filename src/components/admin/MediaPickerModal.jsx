...
export default function MediaPickerModal({ 
  ...
  const fetchMedia = async () => {
    ...
    try {
      ...
      if (selectedUrl && data.length > 0) {
        const found = data.find(m => (m.file_url || m.url) === selectedUrl)
        if (found) {
          selectItem(found)
        }
      }
    } catch (err) {
      console.warn('Failed to load media in modal:', err.message)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSaveNameAndMeta = async (e) => {
    ...
    try {
      ...
      setActiveItem(prev => ({ ...prev, ...updated }))
      setIsEditingMeta(false)
    } catch (err) {
      alert('فشل تحديث اسم وبيانات الصورة: ' + err.message)
    } finally {
      setSavingMeta(false)
    }
  }
  ...
  const handleConfirmSelect = () => {
    if (!activeItem) return
    const url = activeItem.file_url || activeItem.url
    onSelect({
      url,
            ...
            <div className="w-full md:w-80 border-t md:border-t-0 md:border-r border-[#E6E1DC] bg-white p-5 flex flex-col justify-between overflow-y-auto space-y-4">
              <div className="space-y-4">
                <div className="aspect-video bg-[#FAF8F5] rounded-xl overflow-hidden border border-[#E6E1DC] flex items-center justify-center">
                  <img
                    src={activeItem.file_url || activeItem.url}
                    alt={activeItem.alt_text || activeItem.name}

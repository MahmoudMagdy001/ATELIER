      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl border border-[#E6E1DC] shadow-2xl w-full max-w-5xl h-[88vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E6E1DC] bg-[#FAF8F5]">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-[#C4A070]/15 text-[#C4A070] flex items-center justify-center">
              <FaImage className="w-4 h-4" />
            </div>
            <div>
              <h3 className="font-bold text-base text-[#141110]">{title}</h3>
              <p className="text-[11px] text-[#8C7F75]">اختر من صور الموقع أو ارفع صورة جديدة مباشرة مع تحسين الجودة</p>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <label className="px-4 py-2 rounded-xl bg-[#C4A070] text-white hover:bg-[#B88F48] text-xs font-bold flex items-center gap-2 cursor-pointer transition-all shadow-sm">
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-2 rounded-xl hover:bg-[#E6E1DC] text-[#8C7F75] hover:text-[#141110] transition-colors"
            >
        ...
        <div className="px-6 py-3 border-b border-[#E6E1DC] bg-white flex items-center gap-3">
          ...
          <input
            ...
            className="w-full text-xs text-[#141110] focus:outline-none placeholder-[#8C7F75]"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
          />
          {search && (
            <button onClick={() => setSearch('')} className="text-xs text-[#8C7F75] hover:text-[#141110]">
              مسح
            </button>
          )}
        </div>
          ...
          <div className="flex-1 p-4 sm:p-6 overflow-y-auto bg-[#FCFBF9]">
            {loading ? (
              <div className="h-full flex flex-col items-center justify-center text-xs text-[#8C7F75] py-12">
                <div className="w-8 h-8 border-2 border-[#C4A070] border-t-transparent rounded-full animate-spin mb-3"></div>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center p-8 text-[#8C7F75]">
                ...
                <p className="font-bold text-sm text-[#141110]">لم يتم العثور على صور</p>
                  ...
                  return (
                    <div
                      ...
                      className={`group relative bg-white rounded-xl sm:rounded-2xl border overflow-hidden cursor-pointer transition-all ${
                        isSelected
                          ? 'border-[#C4A070] ring-2 ring-[#C4A070] shadow-md'
                          : 'border-[#E6E1DC] hover:border-[#C4A070]/60'
                      }`}
                    >
                      <div className="aspect-square bg-[#FAF8F5] relative overflow-hidden flex items-center justify-center">
                        ...
                        />
                        {isSelected && (
                          <div className="absolute inset-0 bg-[#C4A070]/20 flex items-start justify-end p-2">
                            <span className="w-6 h-6 rounded-full bg-[#C4A070] text-white flex items-center justify-center shadow-md">
                      ...
                      <div className="p-2.5 bg-white">
                        <p className="text-[11px] font-bold text-[#141110] truncate" title={item.name}>
                          {item.name}
                        </p>
            ...
            <div className="w-full md:w-80 border-t md:border-t-0 md:border-r border-[#E6E1DC] bg-white p-5 flex flex-col justify-between overflow-y-auto space-y-4">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <button
                        ...
                        className="text-[10px] text-[#C4A070] hover:underline font-bold flex items-center gap-1"
                      >
                    ...
                    {isEditingMeta ? (
                      <form onSubmit={handleSaveNameAndMeta} className="space-y-2.5 bg-[#FAF8F5] p-3 rounded-xl border border-[#E6E1DC]">
                        <div>
                          ...
                          <input
                            type="text"
                            className="w-full px-2.5 py-1.5 rounded-lg border border-[#E6E1DC] text-xs text-[#141110] bg-white focus:border-[#C4A070] focus:outline-none"
                            value={editName}
                        ...
                        <div className="flex gap-2 pt-1">
                          <button
                            type="submit"
                            disabled={savingMeta}
                            className="flex-1 py-1.5 bg-[#C4A070] text-white text-[11px] font-bold rounded-lg hover:bg-[#B88F48] flex items-center justify-center gap-1"
                          >
                      ...
                      <div className="bg-[#FAF8F5] p-2.5 rounded-xl border border-[#E6E1DC] space-y-1">
                        <p className="text-xs font-bold text-[#141110] break-all">{activeItem.name}</p>
                        {activeItem.alt_text && (
              ...
              <div className="space-y-2 pt-3 border-t border-[#E6E1DC]">
                <button
                  ...
                  className="w-full py-2.5 bg-[#141110] hover:bg-[#26211F] !text-white text-xs font-bold rounded-xl flex items-center justify-center gap-2 shadow-md transition-all cursor-pointer"
                >
                  <FaCheck className="w-3.5 h-3.5 text-[#C4A070]" />
                  <span className="!text-white font-bold">تأكيد واختيار هذه الصورة</span>
                </button>
